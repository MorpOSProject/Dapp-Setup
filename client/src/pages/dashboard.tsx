...
function ShadowBalanceCard() {
  ...
  // Calculate total USD value from holdings with real shadow/public split
  // Calculate USD values per-holding to correctly handle different token prices
  const totals = (holdings || []).reduce((acc, h) => {
    const totalBalance = h.shadowBalance + h.publicBalance;
    // Calculate the per-token price from total value
    const pricePerToken = totalBalance > 0 ? h.valueUsd / totalBalance : 0;
    // Calculate shadow and public USD values for this holding
    const shadowUsd = h.shadowBalance * pricePerToken;
    const publicUsd = h.publicBalance * pricePerToken;
    
    return {
      totalValue: acc.totalValue + h.valueUsd,
      totalShadowValue: acc.totalShadowValue + shadowUsd,
      totalPublicValue: acc.totalPublicValue + publicUsd,
    };
  }, { totalValue: 0, totalShadowValue: 0, totalPublicValue: 0 });
  
  const { totalValue, totalShadowValue, totalPublicValue } = totals;
